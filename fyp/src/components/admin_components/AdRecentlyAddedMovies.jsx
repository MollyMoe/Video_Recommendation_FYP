    ...
    const fetchRecentlyAddedPersistent = async () => {
      ...
      try {
        const batchIdResponse = await axios.get(
          `${API}/api/movies/latest-sync-batch-id`
        );
        ...
        if (latestBatchId) {
          const moviesResponse = await axios.get(
            `${API}/api/movies/recently-added-persistent`,
            {
              params: { batch_id: latestBatchId },
            }
          );
          console.log(
            "📦 Persistent recently added movies response for batch:",
            moviesResponse.data
          );
          setRecentlyAddedPersistent(moviesResponse.data);
        } else {
          console.log(
            "No latest batch ID found, clearing recently added movies."
          );
          setRecentlyAddedPersistent([]);
        }
      } catch (err) {
        console.error(
          "❌ Failed to fetch persistent recently added movies",
          err
        );
        setRecentlyAddedPersistent([]);
      } finally {
  ...
  if (!recentlyAddedPersistent || recentlyAddedPersistent.length === 0) {
    return (
      <div className="text-center text-gray-600 dark:text-gray-400 mt-10">
        ...
        <p className="text-sm">
          Click 'Update' on the 'All Movies' tab (or the side button) to sync
          new movies from the 'added' collection.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="sm:ml-0 px-4 sm:px-8 dark:bg-gray-800 dark:border-gray-700 mr-50">
      <div className="mb-6 mt-10 p-4 bg-green-50 dark:bg-green-800 rounded-lg shadow-inner">
        <h2 className="text-green-700 dark:text-green-200 font-bold text-lg mb-3">
          🆕 Recently Added
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {recentlyAddedPersistent.map((movie) => (
            <div
              key={movie._id}
              className="bg-green-100 dark:bg-green-700 p-2 rounded shadow text-center"
            >
              {/* Movie poster */}
              <img
