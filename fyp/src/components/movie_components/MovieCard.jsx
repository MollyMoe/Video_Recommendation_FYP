...
function MovieCard({ movie = {}, onClick = () => {} }) {
  ...
  return (
    ...
    >
      ...
      {/* Trailer Preview - Only show if trailer exists and isHovered */}
      {isHovered && trailer_key && (
        <div className={`absolute top-9 w-[350px] z-50 pointer-events-none ${trailerAlign}`}>
          <div className="aspect-[5/3] overflow-hidden rounded-t-xl shadow-lg">
            <iframe
              src={`https://www.youtube.com/embed/${trailer_key}?autoplay=1&mute=1&loop=1&playlist=${trailer_key}&controls=0`}
              frameBorder="0"
              allow="autoplay; encrypted-media; fullscreen"
              allowFullScreen
              className="w-full h-full object-cover"
          ...
          <div className="bg-black/60 text-white text-xs p-2 rounded-b-xl space-y-1">
            <div>{Array.isArray(genres) ? genres.join(', ') : genres}</div>
            <div className="font-semibold text-sm">
              ⭐ {predicted_rating ? predicted_rating.toFixed(1) : 'N/A'}
            </div>
          </div>
        </div>
      )}
