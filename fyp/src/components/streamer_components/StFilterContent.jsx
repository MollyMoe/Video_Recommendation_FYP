...
const fetchMovies = async () => {
  ...
  try {
    ...
    const normalizeString = (str) => {
      if (Array.isArray(str)) return str.join(" ").toLowerCase();
      if (typeof str === "string") return str.replace(/[|,]/g, " ").toLowerCase();
      return "";
    };
...
const finalMovieSet = allMovieList.slice(0, 1000).map(movie => {
  ...
  return {
    ...movie,
    trailer_key,
    genres: formatField(movie.genres),
    producers: formatField(movie.producers),
    actors: formatField(movie.actors),
  };
});
    setMovies(finalMovieSet);
  } catch (err) {
