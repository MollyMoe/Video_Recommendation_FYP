...
//                 {/* Hover Info Panel */}
//                 <div className="bg-black/60 text-white text-xs p-2 rounded-b-xl space-y-1 max-h-40 overflow-auto">
...
//                   <strong>Producers:</strong> {Array.isArray(selectedMovie?.producers) ? selectedMovie.producers.join(", ") : "N/A"}
//                 </p>
//                 <p className="text-sm text-gray-700">
//                   <strong>Actors:</strong> {Array.isArray(selectedMovie?.actors) ? selectedMovie.actors.join(", ") : "N/A"}
//                 </p>
          ...
          {movies.map((movie) => (
            ...
            >
              ...
              <div className="absolute left-1/2 top-9 transform -translate-x-1/2 w-[350px] z-10 hidden group-hover:block">
                ...
                {/* Hover Info Panel */}
                <div className="bg-black/60 text-white text-xs p-2 rounded-b-xl space-y-1 max-h-44 overflow-auto">
                  <div><strong>Genres:</strong> {movie.genres?.join(", ")}</div>
                  <div><strong>Director:</strong> {movie.director || "Unknown"}</div>
                  <div><strong>Producers:</strong> {Array.isArray(movie.producers) ? movie.producers.join(", ") : movie.producers || "N/A"}</div>
                  <div><strong>Actors:</strong> {Array.isArray(movie.actors) ? movie.actors.join(", ") : movie.actors || "N/A"}</div>
                  <div><strong>Overview:</strong> {movie.overview || "No description available."}</div>
                  <div className="font-semibold text-sm">
                    ⭐ {movie.predicted_rating?.toFixed(1) || "N/A"}
                  </div>
                </div>
