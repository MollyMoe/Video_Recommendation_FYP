  ...
  try {
    if (isOnline) {
      ...
      if (!newMovies.length) {
        ...
        let currentIndex = parseInt(localStorage.getItem("onlineRegenerateIndex"), 10);
        if (isNaN(currentIndex)) currentIndex = 0;

        // Reset index only if these are completely new recommendations
        if (JSON.stringify(newTitles) !== JSON.stringify(lastRecommendedMovies?.map(m => m.title) || [])) {
          currentIndex = 0;
        }

        localStorage.setItem("onlineRegenerateIndex", currentIndex);
        setRegenerateIndex(currentIndex);
      }
      ...
      // Always start from stored index
      let currentIndex = parseInt(localStorage.getItem("offlineRegenerateIndex"), 10);
      if (isNaN(currentIndex)) currentIndex = 0;
      ...
      // Now increment and store for the *next* regeneration
      const nextIndex = currentIndex + 1;
      localStorage.setItem("offlineRegenerateIndex", nextIndex);
  ...
  const loadOfflineMovies = async () => {
    try {
      ...
      if (!offlinePool || offlinePool.length === 0) {
        throw new Error("No offline movies found");
      }
      ...
      setMovies(nextBatch);
      setLastRecommendedMovies(nextBatch);
      setAllShownTitles(new Set(newTitles));
      setRegenerateIndex(currentIndex);

    } catch (err) {
      console.error("❌ Failed to load offline movies:", err);
    } finally {
      setIsLoading(false); // Ensure loading stops
    }
  };

  if (savedUser?.userId && !isOnline) {
    loadOfflineMovies();
  }
}, [savedUser?.userId, username, isOnline]);
